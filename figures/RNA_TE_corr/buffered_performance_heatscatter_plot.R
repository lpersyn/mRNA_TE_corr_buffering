...
# Only plot the bottom 250 in ObservedCorr
predictions <- predictions %>% 
  arrange(ObservedCorr) %>% 
  slice(1:250)


pdf("./figures/RNA_TE_corr/buffered_predicted_vs_observed_RNA_TE_corr_human.pdf", 5, 5) 
...
pdf("./figures/RNA_TE_corr/buffered_predicted_vs_observed_RNA_TE_corr_mouse.pdf", 5, 5) 
# 'predictions' below is a dataframe with at least two columns named PredictedCorr and ObservedCorr
predictions %>% 
  mutate(
    r2 = cor(PredictedCorr, ObservedCorr)^2,
